...
public static class WebClientInjection
{
    ...
    private const string ClientScriptVersion = "2026.08.14.4";
    ...
    public static string BuildRelativeScriptTag()
        => $"<script id=\"{Marker}\" src=\"configurationpage?name=manual-subtitle-extract.js&amp;v={ClientScriptVersion}\" defer></script>";

    public static string? Inject(string html, string webBasePath)
        => InjectScriptTag(html, BuildScriptTag(webBasePath));

    public static string? InjectScriptTag(string html, string script)
    {
    ...
    public static string TransformIndexHtml(FileTransformationInput? input)
    {
        ...
        return InjectScriptTag(html, BuildRelativeScriptTag()) ?? html;
    }
}
